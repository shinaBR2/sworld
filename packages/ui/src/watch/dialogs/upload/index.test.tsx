...
import type { Videos_Insert_Input } from 'core/graphql/graphql';
...
vi.mock('./utils', () => ({
  ...
  buildVariables: vi
    .fn()
    .mockImplementation((state): { objects: Array<Videos_Insert_Input> } => {
    ...
    <div data-testid="dialog" data-open={open}>
      <form onSubmit={handleSubmit}>
        ...
        <input
          ...
          onChange={onFormFieldChange('url')}
          disabled={state.isSubmitting}
        />
        <input
        ...
        <input
          ...
          onChange={(e) =>
            onFormFieldChange('keepOriginalSource')({
        ...
        <input
          ...
          onChange={(e) =>
            onFormFieldChange('keepDialogOpen')({
              target: { value: e.target.checked },
            })
          }
          disabled={state.isSubmitting}
        ...
        {state.closeDialogCountdown && !state.keepDialogOpen && (
          <div data-testid="countdown">
            Closing in {state.closeDialogCountdown}s
          </div>
        )}
      </form>
    </div>
...
describe('VideoUploadDialog', () => {
  ...
  beforeEach(() => {
    ...
    mockCanPlayUrls.mockResolvedValue([
      { url: 'https://example.com/video.mp4', isValid: true },
    ]);
    ...
    mockBulkConvert = vi
      .fn()
      .mockImplementation(
        async (variables: { objects: Array<Videos_Insert_Input> }) => {
          const result = {
            insert_videos: {
              returning: [{ id: '123', title: 'Test Video' }],
            },
  ...
  it('should render when open is true', async () => {
    renderWithAct(
      <VideoUploadDialog open={true} onOpenChange={mockOnOpenChange} />,
    );
    await waitFor(() => {
