...
describe('Welcome', () => {
  ...
  it('should render loading skeleton when isLoading', () => {
    ...
    render(<Welcome />);
    expect(screen.getByLabelText('Welcome section loading')).toBeInTheDocument();
  ...
  it('should render welcome message with user name', () => {
    mockUseAuthContext.mockReturnValue({ user: { name: 'Alice' }, isLoading: false });
    render(<Welcome />);
    expect(screen.getByLabelText('Welcome section')).toBeInTheDocument();
    expect(screen.getByText('Welcome back, Alice!')).toBeInTheDocument();
    expect(screen.getByText('Continue your reading journey or discover something new.')).toBeInTheDocument();
  });
