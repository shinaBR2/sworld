...
6. **Add a cooldown, and make it long.** A cooldown (pnpm's `minimumReleaseAge`) refuses to resolve
   ...
   window you can tolerate**: 7 days (`10080`) is the recommended conservative default, and `1440`
   (1 day) is the minimum worth setting; when you genuinely need a just-published fix, bypass the wait for that
