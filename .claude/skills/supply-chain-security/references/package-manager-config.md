...
| Cooldown exclusions | the vite/rolldown build toolchain + `postcss`, plus short-lived per-package bypasses for recent security patches — see the file for the current set, each with a written reason | `pnpm-workspace.yaml` |
