...
The generic mutation hook (in `packages/core/src/<domain>/mutation-hooks`) takes a typed Hasura input and forwards it — nothing more. The domain logic — what to copy, reset, or default — lives in the payload builder that produces that input, colocated with the component.
