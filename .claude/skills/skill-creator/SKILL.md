...
- **Repo paths.** Name locations by concept — "the shared core package", "the Hasura metadata", "an app's E2E suite" — rather than pinning a repo path as the skill's record of *where something lives*. That's the most drift-prone fact there is: a refactor moves the folder and the frozen copy rots, while an agent can always rediscover the current location from the code. Two kinds of path are fine, because neither records where code lives. One: a pointer to a reference doc the skill deliberately cross-links — its own co-located `references/` file, or a shared `references/` fact-file. Two: the path *slot* an illustrative command needs to make sense — `docker build -f <dockerfile> .` needs the `-f` argument or it stops reading as a build command, so you show the slot (a placeholder, per *Private values* below) rather than drop it. The target is a *descriptive* path standing in as a source of truth, not every character that happens to be a path.
