...
- **Repo-specific** — a name, flag, or convention that only holds here and can't be guessed. (A *path used to say where something lives* is not this — it's rediscoverable at runtime, so it's left out; see *Two things whose home is outside the skill* below, which also notes the paths that are still kept.)
...
Both have a home outside the skill, so a copy inside it is either drift or a leak:

- **Repo paths.** Name locations by concept — "the shared core package", "the Hasura metadata", "an app's E2E suite" — rather than pinning a repo path as the skill's record of *where something lives*. That's the most drift-prone fact there is: a refactor moves the folder and the frozen copy rots, while an agent can always rediscover the current location from the code. Two kinds of path don't drift and are fine: a pointer to a reference doc it deliberately cross-links — the skill's own co-located `references/` file, or a shared `references/` fact-file — since that pointer *is* the sanctioned cross-link and the doc is a source of truth by design, and a path that's an intrinsic part of an illustrative command (`docker build -f <dockerfile> .`) where dropping it makes the example meaningless. The target is the *descriptive* path standing in as a source of truth, not every character that happens to be a path.
- **Private values.** The repo is public and skills are checked in, so state only the *mechanism* — which config keys exist, that a flag overrides a default — never the *values*: real user IDs, account names, secrets. Those live in local config and memory. Use a placeholder (`--user-id <user-id>`) and, before you finish, scan the diff for every concrete identity or secret value — UUID-shaped strings, real account names or handles, and token/key-shaped strings — not just UUIDs.
