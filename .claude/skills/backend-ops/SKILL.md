...
These CLIs are operator tools run straight from source with `tsx` — they don't go through the container images or the Cloud Run deploy path at all, so a backend deploy (see `references/deployment-model.md`) neither helps nor blocks them.
