...
- **Backend** — nothing deploys `apps/backend` right now. The container images build again (SWO-545), but the workflows that pushed them to Cloud Run were removed in the monorepo move and nothing here yet builds or pushes them — the deploy pipeline is still being rebuilt (see `backend-architecture`). Merging a backend change is currently a no-op in production — the running Cloud Run services keep serving the last image built before the move.
